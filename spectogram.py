...
def stft(sig, frameSize, overlapFac=0.5, window=np.hanning):
    ...
    frames_by_2 = int(frameSize)/2
    frames_zeros = np.zeros(int(np.floor(frames_by_2)))
    samples = np.append(frames_zeros, sig)
    # cols for windowing
    cols = int(np.ceil( (len(samples) - frameSize) / float(hopSize)) + 1)
    # zeros at end (thus samples can be fully covered by frames)
    samples = np.append(samples, np.zeros(frameSize))

    frames = stride_tricks.as_strided(samples, shape=(cols, frameSize), strides=(int(samples.strides[0]*hopSize), int(samples.strides[0]))).copy()
    frames *= win
...
def logscale_spec(spec, sr=44100, factor=20.):
    ...
    for i in range(0, len(scale)):
        scale_i = int(scale[i])
        if i == len(scale)-1:
            newspec[:,i] = np.sum(spec[:,scale_i:], axis=1)
        else:
            scale_i_p_1 = int(scale[i+1])
            newspec[:,i] = np.sum(spec[:,scale_i:scale_i_p_1], axis=1)
    ...
    for i in range(0, len(scale)):
        scale_i = int(scale[i])
        if i == len(scale)-1:
            freqs += [np.mean(allfreqs[scale_i:])]
        else:
            scale_i_p_1 = int(scale[i+1])
            freqs += [np.mean(allfreqs[scale_i:scale_i_p_1])]
